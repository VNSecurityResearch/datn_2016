package com.viettel.dns.period;
...
import scala.Tuple7;
/**
 * The {@link PeriodicallyQueryDetection} detect request periodically. 
 * @author THANHNT78
 * 
 * */
public class PeriodicallyQueryDetection implements Serializable {
	...
	final static Logger logger = Logger.getLogger(PeriodicallyQueryDetection.class);
	...
	public static void main(String[] args) throws IOException{
		PeriodicallyQueryDetection starter =null;

			//starter = new PeriodicallyRequestDetection(args[0]);
		starter = new PeriodicallyQueryDetection("config.txt");
			starter.detect();
	 ...
	 * 
	 * */
	public PeriodicallyQueryDetection(String config_path) throws IOException{
	...
	/**
	 * Create a {@link PeriodicallyQueryDetection} by Properties config
	 * @param config properties of {@link PeriodicallyQueryDetection}
	 * 
	 * */
	public PeriodicallyQueryDetection(Properties config){
		loadConfig(config);
	}
	/**
	...
	public void detect() throws IOException {
		...
		df.printSchema();
		df.show(30);
		// sql_column la cot can loc, sql_value la gia tri loc "category='dns'
		DataFrame analysis = df.filter(sql_column+"='"+sql_value+"'").select("src", "hostname", "dst", "dpt", "timestamp", "qtype", "message")
		...
		DataFrame analysis_maptime = sqlContext
				.sql("SELECT src, hostname, maptime(timestamp) as maptime, message from dataset ").cache();
